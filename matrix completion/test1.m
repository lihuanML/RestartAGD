...
0% gradient descent
eta = 110; % objective increases dramatically when eta=120
...
2% our Algorithm 4
eta = 120; % objective increases dramatically when eta=140;
...
3% Jin's AGD
eta = 40; % eta=50: go to the first case of NCE (i.e., discard the update and go back to the previous iterate) frequently
[tloss3,tgrad3,loss3,grad3,time3] = agd_jin(data_all,I_all,J_all,siz,maxit,eta);

4% convex until proven guilty
eta = 50; % eta=60: NCE frequently, slow
[tloss4,tgrad4,loss4,grad4,time4] = guilty(data_all,I_all,J_all,siz,maxit,eta);

5% empirical AGD
eta = 40; % gradient norm increases and oscilates when eta=50. Also restart frequently
...
hold on;
semilogy(time5(treiter5),tloss5(treiter5)-fmin,'o','MarkerFaceColor','b');
hold on;
set(gca,'Fontsize',20)
xlabel('Time (s)');
