...
while k < KK || B0 > BB                  % step 2 in Algorithm 2
    ...
    [gradU,gradV] = grad_mc(data_train,I_train,J_train,Mat,len,TUcur,TVcur);           % calculate the gradient at (TUcur,TVcur)

    Upre = Ucur;
    Vpre = Vcur;

    Ucur = TUcur-eta*gradU;                                         % gradient descent step at (TUcur,TVcur);   step 4 in Algorithm 2
    ...
    if k*S > max([BB^2,B0^2]) || k>KK                               % step 6 in Algorithm 2
        ...
        curobj = funvalue_mc(data_train,I_train,J_train,len,Ucur,Vcur);   % calculate f(x^k) used in step 8 in Algorithm 2
