...
preobj = funvalue_mc(data_train,I_train,J_train,len,Ucur,Vcur);       % compute the function value
...
for k=1:N                     % run N iterations
    ...
    [gradU,gradV] = grad_mc(data_train,I_train,J_train,Mat,len,TUcur,TVcur);
    ...
    curobj = funvalue_mc(data_train,I_train,J_train,len,Ucur,Vcur);     % compute the function value at (Ucur,Vcur)
    ...
    if mod(k,100) == 0                                      % print every 100 iterations
        disp([ 'Empirical RAGD--iter: ',num2str(k),', gradient: ',num2str(GRAD1(k),15),', obj: ',num2str(OBJ1(k),15) ] );
    end
    tic
end