...
if ~isempty(zerocolidx)
    ...
    M = M(:,fullcol);                                  % remove the 0 columns
end
clear fullcol
[m,n] = size(M);
...
0% gradient descent
eta = 330; % objective increases dramatically when eta=340
[tloss0,tgrad0,loss0,grad0,time0] = gd(data_all,I_all,J_all,siz,maxit,eta);

1% our Algorithm 2
eta = 120; % gradient norm increases dramatically when eta=130
...
2% our Algorithm 4
eta = 370; % objective increases dramatically when eta=380
...
3% Jin's AGD
eta = 140; % eta=150: go to the first case of NCE (i.e., discard the update and go back to the previous iterate) frequently. slow
...
5% empirical AGD
eta = 120; % gradient norm increases and oscilates when eta=130. Also restart frequently
...
eta = 300; % no effect due to line search
