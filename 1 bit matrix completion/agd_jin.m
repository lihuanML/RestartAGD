...
theta = 0.04*(eps*rho*eta^2)^(1/4);  % (3) in Jin2018 suggests theta = 0.5*(eps*rho*eta^2)^(1/4).
                                     % we tune it for better performance on matrix completion problem and Movie10 dataset;
...
for k = 1:N                          % run N iterations
    ...
    [gradU,gradV,obj] = grad_funvalue_1mc(data_train,I_train,J_train,Mat,len,TUcur,TVcur);
    ...
    fx = funvalue_1mc(data_train,I_train,J_train,len,Upre,Vpre);                % calculate the objective function value at (Upre,Vpre)
    ...
    if fx < fy + sum(sum(gradU.*(Upre-TUcur))) + sum(sum(gradV.*(Vpre-TVcur))) - gamma/2*norm(Upre-TUcur,'fro')^2 - gamma/2*norm(Vpre-TVcur,'fro')^2      %  check NCE condition
      if sqrt( norm(DiffUpre,'fro')^2+norm(DiffVpre,'fro')^2 ) >= s             % case 1
          ...
          Ucur1 = Upre + Udelta;
          Vcur1 = Vpre + Vdelta;
          f1 = funvalue_1mc(data_train,I_train,J_train,len,Ucur1,Vcur1);
    
          Ucur2 = Upre - Udelta;
          Vcur2 = Vpre - Vdelta;
          f2 = funvalue_1mc(data_train,I_train,J_train,len,Ucur2,Vcur2);
