...
0% gradient descent
eta = 640; % objective increases dramatically when eta=650
           % eta=630 and eta=640 has little effect on the performance, but we still tune the best one. The other methods are similar. 
[tloss0,tgrad0,loss0,grad0,time0] = gd(data_all,I_all,J_all,siz,maxit,eta);

1% our Algorithm 2
eta = 130; % objective increases dramatically when eta=140
...
2% our Algorithm 4
eta = 300; % objective increases dramatically when eta=310
...
3% Jin's AGD
eta = 180; % eta=190: go to the first case of NCE (i.e., discard the update and go back to the previous iterate) frequently and gradient norm increases dramatically
[tloss3,tgrad3,loss3,grad3,time3] = agd_jin(data_all,I_all,J_all,siz,maxit,eta);

4% convex until proven guilty
eta = 140; % eta=150: NCE frequently and gradient norm increases dramatically
[tloss4,tgrad4,loss4,grad4,time4] = guilty(data_all,I_all,J_all,siz,maxit,eta);

5% empirical AGD
eta = 90;  % restart frequently and gradient norm increases dramatically when eta=100
[tloss5,tgrad5,loss5,grad5,time5,treiter5,reiter5] = emp_ragd(data_all,I_all,J_all,siz,maxit,eta);

6% conjugate gradient descent
eta = 500;  % no effect due to line search
