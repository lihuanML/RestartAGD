...
while k < KK || B0 > BB                 % step 2 in Algorithm 4
    ...
    [gradU,gradV] = grad_1mc(data_train,I_train,J_train,Mat,len,Ucur,Vcur);
    ...
    if k*S > max([BB^2,B0^2]) || k>KK                                                       % step 5 in Algorithm 4
        ...
        curobj = funvalue_1mc(data_train,I_train,J_train,len,ZUcur,ZVcur);                  % calculate f(z^k) used in step 8 in Algorithm 4
        ...
        reiter2(t) = itercount;
        if B0 <= BB && biter1 == -1               
            biter1 = iter;                                                                  % record where B0 decreases smaller than B for the first time
            biter2 = itercount;
        end
